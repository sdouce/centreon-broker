...
#include "com/centreon/broker/dump/factory.hh"
#include <cstring>
#include <json11.hpp>
#include <memory>
#include <sstream>
...
static std::string find_param(config::endpoint const& cfg,
  ...
  if (cfg.params.end() == it)
    throw exceptions::msg() << "dump: no '" << key
...
bool factory::has_endpoint(config::endpoint& cfg, flag* flag) {
  bool is_ifdb{!strncasecmp(cfg.type.c_str(), "dump", 5)};
  if (flag)
    *flag = no;
  /*if (is_ifdb) {
    cfg.params["filename"] = "yes";
    cfg.cache_enabled = true;
  }*/

  std::cout << cfg.type.c_str() << " " << is_ifdb << std::endl;
  return is_ifdb;
}
...
io::endpoint* factory::new_endpoint(
  ...
  std::string filename(find_param(cfg, "filename"));
    ...
    if (it == cfg.params.end())
      ...
      if (!ss.eof())
        throw exceptions::msg()
            << "dump: couldn't parse port '" << ss.str()
            << "' defined for endpoint '" << cfg.name << "'";
    }
  }
  ...
  uint32_t queries_per_transaction(0);
  {
    ...
    if (it != cfg.params.end())
      try {
        ...
        throw exceptions::msg()
            << "dump: couldn't parse queries_per_transaction '"
  ...
  auto chk_str = [](Json const& js) -> std::string {
    if (!js.is_string() || js.string_value().empty()) {
      throw exceptions::msg()
          << "dump: couldn't get the configuration of a metric column name";
    }
  ...
  std::unique_ptr<dump::connector> c(new dump::connector);
